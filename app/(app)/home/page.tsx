...
export default async function HomePage() {
  ...
  const todaysLessonCourse = inProgress[0] ?? courses.find((c) => c.slug === "chatgpt-basics") ?? courses[0];
      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <Display as="h1" size="md" className="mt-1">
            Hello, <em className="italic font-normal">{firstName(profile?.display_name ?? user.email)}</em>.
          </Display>
        </div>
      ...
      {todaysLessonCourse ? (
        <section className="mt-8">
          ...
          <Link
            ...
            className="mt-3 block rounded-3xl border border-paper-200 bg-paper-100 shadow-paper p-6 sm:p-7 transition-[transform,box-shadow] duration-220 ease-warm hover:-translate-y-[2px] hover:shadow-paper-lg"
          >
            <div className="flex items-start gap-4">
              <TutorAvatar personaId={(profile?.preferred_tutor_persona as "nova" | "arjun" | "riya" | "sensei" | undefined) ?? "nova"} size="lg" />
              <div className="flex-1 min-w-0">
                <p className="font-serif text-ink-700 italic text-[15px]">
                  "Let's pick up where we left off — {todaysLessonCourse.title}."
                </p>
                <p className="mt-3 font-serif text-xl text-ink-900">{todaysLessonCourse.subtitle ?? todaysLessonCourse.title}</p>
              ...
              <ArrowRight className="h-5 w-5 text-ember-500 shrink-0 mt-1" />
            </div>
          </Link>
        </section>
      ) : null}
  ...
  return (
    <section className="mt-10">
      <div className="flex items-baseline justify-between">
        ...
        <Link href="/learn" className="text-xs text-ink-500 hover:text-ink-800 transition-colors">
          more
        </Link>
      </div>
      <div className="mt-3 -mx-5 px-5 flex gap-4 overflow-x-auto no-scrollbar snap-x snap-mandatory">
        {courses.map((c) => (
